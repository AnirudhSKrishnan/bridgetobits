          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="grid gap-2">
                ...
                <input required className="input bg-black text-white border border-yellow-400 min-h-[48px] text-base" placeholder="First name" autoComplete="given-name" />
              </label>
              <label className="grid gap-2">
                <span className="text-sm font-medium" style={{ color: '#fac203' }}>Last Name</span>
                <input required className="input bg-black text-white border border-yellow-400 min-h-[48px] text-base" placeholder="Last name" autoComplete="family-name" />
              </label>
            </div>
            <label className="grid gap-2">
              <span className="text-sm font-medium" style={{ color: '#fac203' }}>Email Address</span>
              <input type="email" required className="input bg-black text-white border border-yellow-400 min-h-[48px] text-base" placeholder="you@example.com" autoComplete="email" pattern="^[^@\s]+@[^@\s]+\.[^@\s]+$" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm font-medium" style={{ color: '#fac203' }}>Phone Number</span>
              <input type="tel" required className="input bg-black text-white border border-yellow-400 min-h-[48px] text-base" placeholder="+91 9876543210" autoComplete="tel" pattern="^\+\d{1,4}\s?\d{10}$" maxLength={15} minLength={10} />
            </label>
            <label className="grid gap-2">
              <span className="text-sm font-medium" style={{ color: '#fac203' }}>Message</span>
              <textarea required className="input bg-black text-white border border-yellow-400 min-h-[120px] text-base resize-y" rows={5} placeholder="How can we help?" />
            </label>
            <button className="btn btn-primary w-full py-3 text-base sm:text-lg mt-2 min-h-[52px]" style={{ background: '#FFD600', color: '#111', border: 'none', fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 'bold' }}>Send Message</button>
            {status === "sent" && (
